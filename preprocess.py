...
def data_preprocessing_A2(images_dir, labels_path, images_test_dir, labels_test_path):
    ...
    cartoon_test_df = pd.read_csv(os.path.normcase(labels_test_path), sep='\t', engine='python')
    df_test = cartoon_test_df[['img_name', 'smiling']]

    # Convert the face shape column to class 1-5
    df.loc[:, 'smiling'] += 1
    df_test.loc[:, 'smiling'] += 1
...
def data_preprocessing_B1(images_dir, labels_path, images_test_dir, labels_test_path):
    # Converting csv into dataframe using read_csv(label_path)
    cartoon_df = pd.read_csv(os.path.normcase(labels_path), sep='\t', engine='python')
    df = cartoon_df[['file_name', 'face_shape']]
    cartoon_test_df = pd.read_csv(os.path.normcase(labels_test_path), sep='\t', engine='python')
    df_test = cartoon_test_df[['file_name', 'face_shape']]

    # Convert the face shape column to class 1-5
    df.loc[:, 'face_shape'] += 1
    df_test.loc[:, 'face_shape'] += 1
...
def data_preprocessing_B2(images_dir, labels_path, images_test_dir, labels_test_path):
    # Converting csv into dataframe using read_csv(label_path)
    cartoon_df = pd.read_csv(os.path.normcase(labels_path), sep='\t', engine='python')
    df = cartoon_df[['file_name', 'eye_color']]
    cartoon_test_df = pd.read_csv(os.path.normcase(labels_test_path), sep='\t', engine='python')
    df_test = cartoon_test_df[['file_name', 'eye_color']]

    # Convert the face shape column to class 1-5
    df.loc[:, 'eye_color'] += 1
    df_test.loc[:, 'eye_color'] += 1
